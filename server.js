...
// --- NEW: Community Channel Collections ---
let channelsCollection; // Stores channel definitions (name, owner, etc.)
let channelPostsCollection; // Stores the ephemeral 24h posts
let followersCollection; // Stores the follow relationships (pubKey <-> channelId)
...
async function connectToMongo() {
  try {
    ...
    // --- NEW: Setup for Community Channels ---
    channelsCollection = db.collection("channels");
    channelPostsCollection = db.collection("channelPosts");
    followersCollection = db.collection("followers");
    
    // 1. Index for finding channels by their owner
    await channelsCollection.createIndex({ ownerPubKey: 1 });
    
    // 2. CRITICAL: TTL Index for 24-hour post expiry
    // This tells MongoDB to automatically delete a document when its 'expireAt' date is reached.
    await channelPostsCollection.createIndex({ "expireAt": 1 }, { expireAfterSeconds: 0 });
    
    // 3. Index for finding posts for a specific channel, sorted by time
    await channelPostsCollection.createIndex({ channelId: 1, timestamp: -1 });
    
    // 4. Indexes for the follow system
    await followersCollection.createIndex({ pubKey: 1 }); // Find channels a user follows
    await followersCollection.createIndex({ channelId: 1 }); // Find followers of a channel
    await followersCollection.createIndex({ pubKey: 1, channelId: 1 }, { unique: true }); // Prevent duplicate follows
    
    console.log("✅ Community Channels collections and indexes are ready.");
    // --- END NEW ---
  } catch (err) {
...
socket.on("signal", ({ room, payload }) => {
  ...
  // --- NEW: Community Channel Socket API ---

// Helper function to generate a safe channel ID from a name
...
socket.on("community:create", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated. Please register.' });
    const { channelName, description, avatar } = data;
    if (!channelName) return callback({ error: 'Channel name is required.' });

    // Check for profanity or basic validation
    if (channelName.length < 3 || channelName.length > 50) {
        return callback({ error: 'Channel name must be between 3 and 50 characters.' });
    }

    let channelId = generateChannelId(channelName);

    try {
        // Check if this user already owns a channel
        const existingChannel = await channelsCollection.findOne({ ownerPubKey: socket.data.pubKey });
        if (existingChannel) {
            return callback({ error: 'You can only create one channel per account.' });
        }

        // Check if this ID is taken
        const idTaken = await channelsCollection.findOne({ _id: channelId });
        if (idTaken) {
            channelId = `${channelId}-${Date.now().toString().slice(-5)}`; // Make unique
        }

        const newChannel = {
            _id: channelId,
            ownerPubKey: socket.data.pubKey,
        ...
        console.log(`✅ Channel Created: ${channelId} by ${socket.data.pubKey.slice(0,10)}`);
        callback({ success: true, channel: newChannel });

    } catch (err) {
        console.error("community:create error:", err);
        callback({ error: 'Server error creating channel.' });
    }
});

// 2. Follow a channel
socket.on("community:follow", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated.' });
    const { channelId, follow } = data; // follow is true or false
    ...
    try {
        if (follow) {
            ...
            await followersCollection.updateOne(
                { pubKey: socket.data.pubKey, channelId: channelId },
                { $setOnInsert: { pubKey: socket.data.pubKey, channelId: channelId, followedAt: new Date() } },
                { upsert: true }
            );
            console.log(`[Follow] ${socket.data.pubKey.slice(0,10)} followed ${channelId}`);
            callback({ success: true, status: 'followed' });
        } else {
            // Unfollow
            await followersCollection.deleteOne({ pubKey: socket.data.pubKey, channelId: channelId });
            console.log(`[Unfollow] ${socket.data.pubKey.slice(0,10)} unfollowed ${channelId}`);
            callback({ success: true, status: 'unfollowed' });
        }
    } catch (err) {
        console.error("community:follow error:", err);
        callback({ error: 'Server error.' });
    }
});

// 3. Post a message to a channel
socket.on("community:post", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated.' });
    const { channelId, messageContent, messageType } = data;
    const POST_LIMIT_MB = 2;

    if (!channelId || !messageContent) {
        return callback({ error: 'Missing channelId or messageContent.' });
    }

    try {
        // 1. Verify Ownership
        const channel = await channelsCollection.findOne({ _id: channelId, ownerPubKey: socket.data.pubKey });
        if (!channel) {
            return callback({ error: 'You do not own this channel or it does not exist.' });
        }

        // 2. Check 2MB Size Limit
        const messageSizeBytes = Buffer.byteLength(messageContent, 'utf8');
        if (messageSizeBytes > (POST_LIMIT_MB * 1024 * 1024)) {
            return callback({ error: `Message exceeds the ${POST_LIMIT_MB}MB limit.` });
        }

        // 3. Insert the post with 24-hour expiry
        const newPost = {
            channelId: channelId,
            timestamp: new Date(),
            messageContent: messageContent,
            messageType: messageType || "text",
            expireAt: new Date(Date.now() + 24 * 60 * 60 * 1000) // 24-hour TTL
        };

        const result = await channelPostsCollection.insertOne(newPost);
        console.log(`[Post] New post ${result.insertedId} in channel ${channelId}`);
        callback({ success: true, post: newPost });

    } catch (err) {
        console.error("community:post error:", err);
        callback({ error: 'Server error posting message.' });
    }
});

// 4. Delete a post from a channel
socket.on("community:deletePost", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated.' });
    const { postId } = data;
    if (!postId) return callback({ error: 'postId is required.' });
    ...
    try {
        ...
        // 1. Find the post
        const post = await channelPostsCollection.findOne({ _id: _id });
        if (!post) return callback({ error: 'Post not found.' });

        // 2. Verify Ownership of the *channel*
        const channel = await channelsCollection.findOne({ _id: post.channelId, ownerPubKey: socket.data.pubKey });
        if (!channel) {
            return callback({ error: 'You are not the owner of this post.' });
        }

        // 3. Delete the post
        await channelPostsCollection.deleteOne({ _id: _id });
        console.log(`[Delete Post] ${postId} deleted by owner ${socket.data.pubKey.slice(0,10)}`);
        callback({ success: true });

    } catch (err) {
        console.error("community:deletePost error:", err);
        callback({ error: 'Server error or invalid post ID.' });
    }
});

// 5. Get list of all channels (for Explore page)
socket.on("community:getList", async (data, callback) => {
    // No auth needed for this, it's public
    try {
        const channels = await channelsCollection.find({}, {
            projection: { ownerPubKey: 0 } // Don't send owner's key
        }).toArray();
        callback({ success: true, channels: channels });
    } catch (err) {
        console.error("community:getList error:", err);
        callback({ error: 'Server error.' });
    }
});

// 6. Get updates for followed channels (the "pull")
socket.on("community:getUpdates", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated.' });
    const { followedChannels } = data; // e.g., [{ id: 'syrja_dev_blog', lastFetched: 123456789 }]

    if (!followedChannels || !Array.isArray(followedChannels)) {
        return callback({ error: 'Invalid request format.' });
    }

    try {
        const updates = {};
        // Run all queries in parallel
        await Promise.all(followedChannels.map(async (channel) => {
            const posts = await channelPostsCollection.find({
                channelId: channel.id,
                timestamp: { $gt: new Date(channel.lastFetched) }
            }).sort({ timestamp: 1 }).toArray(); // Get oldest first

            updates[channel.id] = posts;
        }));

        callback({ success: true, updates: updates });

    } catch (err) {
        console.error("community:getUpdates error:", err);
        callback({ error: 'Server error.' });
    }
});

// 7. Get owner's own active posts (for the Channel Info panel)
socket.on("community:getMyPosts", async (data, callback) => {
    if (!socket.data.pubKey) return callback({ error: 'Not authenticated.' });
    ...
    try {
        ...
        const channel = await channelsCollection.findOne({ _id: channelId, ownerPubKey: socket.data.pubKey });
        if (!channel) {
            return callback({ error: 'You do not own this channel.' });
        }

        // Find all active posts
        const posts = await channelPostsCollection.find({
            channelId: channelId
        }).sort({ timestamp: -1 }).toArray(); // Newest first

        callback({ success: true, posts: posts });

    } catch (err) {
        console.error("community:getMyPosts error:", err);
        callback({ error: 'Server error.' });
    }
});

// --- END: Community Channel Socket API ---
